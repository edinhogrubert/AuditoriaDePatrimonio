...
import { ArrowLeft, Tag, RefreshCw, Check, Upload, Image as ImageIcon, QrCode, Sparkles } from 'lucide-react';
...
export const QrImportScannerScreen: React.FC<QrImportScannerScreenProps> = ({
  ...
  const parseQrContent = (content: string, delimiter: string) => {
    // If a specific delimiter like ; or , is chosen, we still want to consider newlines
    // as separators to be more robust.
    const separators = new Set(['\n', '\r', delimiter]);
    const regex = new RegExp(`[${Array.from(separators).join('')}]`);
  ...
  const parsedItems = scannedContent ? parseQrContent(scannedContent, selectedDelimiter) : [];
    ...
    reader.onload = async () => {
      ...
      img.onload = async () => {
        try {
          ...
          if (res && res.getText()) {
            setScannedContent(res.getText());
          } else {
            setUploadError('Nenhum QR code foi encontrado nesta imagem.');
          }
        } catch (err) {
          setUploadError('Não foi possível ler um QR Code válido na imagem selecionada. Tente uma foto com melhor nitidez.');
        }
      };
    };
  ...
  const handleConfirmImport = () => {
    ...
    let expectedList = parsedItems.map((barcode) => ({
      barcode: barcode.trim(),
      description: globalName.trim() || 'Item de Inventário',
      category: globalCategory.trim() || 'Sem Categoria'
    }));
