      ...
      <div className="absolute top-0 inset-x-0 z-30 p-4 bg-gradient-to-b from-black/80 to-transparent flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-[var(--color-emerald)] font-bold uppercase tracking-widest drop-shadow-md">
      ...
      <div className="relative flex-1 w-full h-full">
        ...
        {lastScanned && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 translate-y-20 z-20 pointer-events-none bg-black/60 backdrop-blur-md px-5 py-2.5 rounded-2xl border-2 border-emerald-500/40 shadow-2xl scale-110">
            <span className="text-base font-black text-[var(--color-emerald)] font-mono tracking-tighter">
              {lastScanned}
            </span>
          </div>
        )}
      ...
      <div className="absolute bottom-6 inset-x-6 z-30 flex flex-col gap-5">
        ...
        {scanItems.length > 0 && (
          <div className="space-y-2">
            ...
            <div className="bg-[var(--bg-secondary)]/90 backdrop-blur-xl border border-[var(--border-color)] rounded-2xl p-4 max-h-40 overflow-y-auto space-y-3 shadow-2xl">
              {scanItems.slice(0, 5).map((item) => {
                const badge = getTypeLabel(item.format);
                return (
                  <div key={item.id} className="flex items-center justify-between gap-3 text-xs py-1 border-b border-[var(--border-color)]/40 last:border-0 transition-colors">
                    <span className="text-[9px] font-black text-[var(--color-emerald)] font-mono shrink-0">{badge}</span>
        ...
        <div className="flex items-center justify-center gap-10">
          ...
          <div className="w-20 h-20 rounded-full border-4 border-white/20 flex items-center justify-center shadow-2xl bg-white/5 backdrop-blur-sm">
            <div className="w-14 h-14 rounded-full bg-emerald-500/10 flex items-center justify-center border border-emerald-500/20">
              <QrCode className="w-6 h-6 text-[var(--color-emerald)]/60" />
            </div>
          </div>
          ...
          <button
            ...
            className="w-16 h-16 rounded-full bg-[var(--color-emerald)] text-[var(--bg-primary)] shadow-[var(--card-shadow)]"
            title="Finalizar"
          >
